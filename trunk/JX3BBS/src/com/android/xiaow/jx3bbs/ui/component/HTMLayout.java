 ...
 * 
 */
public class HTMLayout extends LinearLayout implements
		ImageViewObserver.Callback {
	...
	public void loadView() {
		...
		for (int i = 0; i < len; i++) {
			...
			if (str.matches(regex_img_0)) {
				...
				imv.setBackgroundResource(R.drawable.back);
				String url = "";
		...
		public Drawable getDrawable(String source) {
			if (!TextUtils.isEmpty(source)) {
				...
				UrldrawableObserver observer = new UrldrawableObserver(view,
						urlDrawable);
				SyncLoadImage.getIntance().LoadBitmap(source, observer,
						SyncLoadImage.SMILE);
				return urlDrawable;
			}
			return null;
