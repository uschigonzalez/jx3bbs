...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        int default_dimen = getResources().getDimensionPixelSize(R.dimen.padding_large);
        mCellWidth = a.getDimensionPixelSize(R.styleable.CellLayout_cellWidth, 10);
        mCellHeight = a.getDimensionPixelSize(R.styleable.CellLayout_cellHeight, 10);

        mLongAxisStartPadding = a.getDimensionPixelSize(
                R.styleable.CellLayout_longAxisStartPadding, default_dimen);
        mLongAxisEndPadding = a.getDimensionPixelSize(R.styleable.CellLayout_longAxisEndPadding,
                default_dimen);
        mShortAxisStartPadding = a.getDimensionPixelSize(
                R.styleable.CellLayout_shortAxisStartPadding, default_dimen);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // mCellWidth = (widthSpecSize - shortAxisEndPadding
        // - shortAxisStartPadding - numShortGaps * 13)
        // / shortAxisCells;
        ...
        if (mPortrait) {
            ...
            mCellHeight = (heightSpecSize - longAxisStartPadding - longAxisEndPadding - (numLongGaps * mHeightGap))
                    / longAxisCells;
            mCellWidth = (widthSpecSize - shortAxisEndPadding - shortAxisStartPadding - (numShortGaps * mWidthGap))
                    / shortAxisCells;
            ...
            mCellWidth = (widthSpecSize - longAxisStartPadding - longAxisEndPadding - (numLongGaps * mWidthGap))
                    / longAxisCells;
            mCellHeight = (heightSpecSize - shortAxisEndPadding - shortAxisStartPadding - (numShortGaps * mHeightGap))
                    / shortAxisCells;
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (lp.dropped) {
                    ...
                    mWallpaperManager.sendWallpaperCommand(getWindowToken(), "android.home.drop",
                            cellXY[0] + childLeft + lp.width / 2, cellXY[1] + childTop + lp.height
                                    / 2, 0, null);
                }
            }
        }
    ...
    /*
     * public CellAdapter getAdapter() { return mAdapter; }
     */

    Animation mCellAnimation;

    public Animation getCellAnimation() {
        return mCellAnimation;
    }

    public void setCellAnimation(Animation mCellAnimation) {
        this.mCellAnimation = mCellAnimation;
    }

    /*
     * public void setAdapter(CellAdapter mAdapter) { this.mAdapter = mAdapter;
     * initAdapte(); mAdapter.registerDataSetObserver(new cellDataObserver()); }
     */
    public void setChangeView(boolean flag) {
        this.childState = flag;
    }
    ...
    /*
     * class cellDataObserver extends DataSetObserver {
     * 
     * @Override public void onChanged() { super.onChanged(); if (childState) {
     * int[] vacant = new int[2]; int size = mAdapter.getCount() <
     * getChildCount() ? getChildCount() : mAdapter.getCount(); for (int i = 0;
     * i < size; i++) { if (i < getChildCount()) mAdapter.initCellChildView(i,
     * getChildAt(i)); else if (i < mAdapter.getCount()) { View view =
     * mAdapter.getView(i, null, null); CellInfo cellInfo =
     * findAllVacantCells(null, null); int[] spanXY =
     * mAdapter.getCellLayoutParams(i); if (cellInfo.findCellForSpan(vacant,
     * spanXY[0], spanXY[1])) { addView(view, -1, new CellLayout.LayoutParams(
     * vacant[0], vacant[1], spanXY[0], spanXY[1])); } } else { removeViewAt(i);
     * } } } else { removeAllViews(); initAdapte(); } }
     * 
     * @Override public void onInvalidated() { super.onInvalidated(); }
     * 
     * };
     * 
     * private void initAdapte() { int[] vacant = new int[2]; for (int i = 0; i
     * < mAdapter.getCount(); i++) { View view = mAdapter.getView(i, null,
     * null); CellInfo cellInfo = findAllVacantCells(null, null); int[] spanXY =
     * mAdapter.getCellLayoutParams(i); if (cellInfo.findCellForSpan(vacant,
     * spanXY[0], spanXY[1])) { addView(view, -1, new
     * CellLayout.LayoutParams(vacant[0], vacant[1], spanXY[0], spanXY[1])); } }
     * }
     * 
     * public void removeOtherViewByPostion(int postion) { int nLen =
     * getChildCount(); for (int i = 0; i < nLen; i++) { View view =
     * getChildAt(i); AreaItemAdpter.Holder holder = (Holder) view.getTag(); if
